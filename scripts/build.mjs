...
md.renderer.rules.fence = (tokens, idx, options, env, self) => {
  ...
  if (token.info.trim()==='mermaid') {
    ...
    const compact=bounds&&bounds[2]<=540?' compact-diagram':'';
    return `<figure class="mermaid-figure${compact}"><a class="diagram-link" href="assets/diagrams/${name}.svg" target="_blank" rel="noopener" aria-label="Open diagram at full size"><img src="assets/diagrams/${name}.svg"${dimensions} alt="${esc(token.content.match(/accTitle:\s*(.*)/)?.[1]||'Concept flowchart')}"></a><figcaption>Mermaid diagram · <a href="assets/diagrams/${name}.svg" target="_blank" rel="noopener">Open full size</a></figcaption><details class="diagram-source"><summary>View Mermaid source</summary><pre><code>${esc(token.content)}</code></pre></details></figure>`;
  }
...
fs.appendFileSync('dist/assets/search-index.js','\nwindow.BOOK_NOTATION='+JSON.stringify(JSON.parse(fs.readFileSync('book/notation-panel.json','utf8'))).replace(/</g,'\\u003c')+';');
